...
public class MezziDao {
	...
	TrattaDao td = new TrattaDao(em1);
...
//**************METODO PER SALVARE MEZZI - POLIMORFISMO***********************
	...
	// RANDOM PER OGNI MEZZO***********************
	public void mezzoCorsa(int number) {
		EntityTransaction t = em.getTransaction();
		t.begin();
		Random rnd = new Random();
			...
			for (int i = 0; i < number; i++) {
				if (mezzo.getN_totale() % 10 == 0 && mezzo.getN_totale() != 0) {
					...
					try {
						...
						log.info("Storico Tratta relativo al mezzo " + mezzo.getCodice_mezzo()
								+ " inserito con successo");
